...
lat.data.2013 <- lat.data.2013[!duplicated(lat.data.2013), ]

# create time variables
time <- as.POSIXlt(lat.data.2013$date)
year <- time$year + 1900
month <- as.factor(months(time))
levels(month) <- month.name[month.name %in% levels(month)]
mday <- time$mday
lat.data.2013 <- data.frame(lat.data.2013, year, month, mday)
```
...
doubley <- doubleYScale(rain, sat, add.ylab2 = TRUE, under = TRUE,
                        text = c( "Precipitation (in)", "% Saturation"), column = 2, 
                        type = c("l", "p"))
update(doubley,
       par.settings = simpleTheme(col = c("black", "red"), pch = c(NA, 16)))
...
ggplot(long.gg.data, aes(x = date, y = sat, size = value)) + facet_grid(variable ~ .) +
  geom_point(color = "steelblue") + scale_size_continuous(range = c(3,8)) + ylab("% Saturation") +
